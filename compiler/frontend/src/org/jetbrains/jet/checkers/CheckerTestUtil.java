...
public class CheckerTestUtil {
    ...
    private static void compareDiagnostics(
        ...
        Multiset<TextDiagnostic> actualDiagnostics = currentActual.getTextDiagnostics();
        ...
        for (TextDiagnostic expectedDiagnostic : expectedDiagnostics) {
            ...
            for (TextDiagnostic actualDiagnostic : actualDiagnostics) {
                if (expectedDiagnostic.getName().equals(actualDiagnostic.getName())) {
                    if (!compareTextDiagnostic(expectedDiagnostic, actualDiagnostic)) {
                        callbacks.wrongParametersDiagnostic(expectedDiagnostic, actualDiagnostic, expectedStart, expectedEnd);
                    }

                    actualDiagnostics.remove(actualDiagnostic);
                    diagnosticToInput.put(actualDiagnostic.diagnostic, expectedDiagnostic);
        ...
        for (TextDiagnostic unexpectedDiagnostic : actualDiagnostics)
    ...
    private static class DiagnosticDescriptor {
        ...
        public Multiset<TextDiagnostic> getTextDiagnostics() {
            Multiset<TextDiagnostic> actualDiagnosticTypes = HashMultiset.create();
            for (Diagnostic diagnostic : diagnostics) {
                actualDiagnosticTypes.add(TextDiagnostic.asTextDiagnostic(diagnostic));
            }
    ...
    public static class TextDiagnostic {
        private final static Pattern DIAGNOSTIC_PATTERN =
                Pattern.compile("(\\w+)(\\(" + DIAGNOSTIC_PARAMETER + "(,\\s*" + DIAGNOSTIC_PARAMETER + ")*\\))?");
        private final static Pattern INDIVIDUAL_PARAMETER_PATTERN = Pattern.compile(DIAGNOSTIC_PARAMETER);

        @NotNull
        private static TextDiagnostic parseDiagnostic(String text) {
            Matcher matcher = DIAGNOSTIC_PATTERN.matcher(text);
            if (!matcher.find())
            ...
            if (parameters == null)
                return new TextDiagnostic(name, null, null);
            ...
            return new TextDiagnostic(name, parsedParameters, null);
        }
        ...
        public static TextDiagnostic asTextDiagnostic(@NotNull Diagnostic diagnostic) {
            ...
            return new TextDiagnostic(diagnosticName, null, diagnostic);
        }
        ...
        public TextDiagnostic(@NotNull String name, @Nullable List<String> parameters, @Nullable Diagnostic diagnostic) {
            this.name = name;
